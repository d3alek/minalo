...
import time
import sh
from sh import head, grep, gpg2
import os
from помощни import време_клон, СЛУШАНЕ, сега, вземи_водачи, изчисли_водачи, вземи_съучастници

os.environ['GNUPGHOME'] = os.getcwd() + '/тайник'
аз = head(grep(gpg2('--with-colons', '-K'), 'fpr'), '-n1').split(':')[9]
...
def минута():
    while True:
        ...
        #    continue
        клон_шаблон = време_клон()
        кандидат_клон_шаблон = време_клон(кандидат=True)
        водачи = вземи_водачи()
        съм_водач = False
        for водач in водачи:
